...
# برای جلوگیری از پاک شدن اطلاعات در سرور ابری رندر، دیتابیس را در پوشه دیسک ذخیره می‌کنیم.
# اگر پوشه /data وجود نداشته باشد (مثلاً در پایدروید یا سیستم خودتان)، در همان مسیر اصلی ساخته می‌شود.
if os.path.exists("/data"):
    DB_NAME = "/data/superleague_v2_pro.db"
else:
    DB_NAME = "superleague_v2_pro.db"
...
def login():
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM coaches WHERE login_code = ?", (code,))
    coach = cursor.fetchone()
    
    if coach:
        cursor.execute("SELECT tournament_locked, league_locked FROM account_permissions WHERE account_type = ?", (coach[6],))
...
def admin_create_team():
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO coaches (login_code, team_name) VALUES (?, ?)", (code, t_name))
    conn.commit()
    conn.close()
...
def admin_update_coach():
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("UPDATE coaches SET account_type=?, coins=?, credit=? WHERE login_code=?", (acc_type, coins, credit, code))
    conn.commit()
    conn.close()
...
def admin_delete_team(codeextend):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM coaches WHERE login_code = ?", (codeextend,))
    conn.commit()
    conn.close()
...
def get_players():
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT player_name, card_url FROM players WHERE team_code=?", (code,))
...
def buy_lucky_box():
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT coins, team_name FROM coaches WHERE login_code=?", (code,))
    coach = cursor.fetchone()
    ...
    cursor.execute("UPDATE coaches SET coins=? WHERE login_code=?", (new_coins, code))
    cursor.execute("INSERT INTO players (team_code, player_name) VALUES (?, ?)", (code, win["name"]))
    cursor.execute("INSERT INTO admin_logs (team_name, log_type, message) VALUES (?, 'باکس شانسی', ?)", (coach[1], f"باز کردن باکس و دریافت بازیکن {win['name']}"))
    conn.commit()
    conn.close()
...
def buy_direct():
    ...
    if coach and coach[0] >= price:
        cursor.execute("UPDATE coaches SET coins=? WHERE login_code=?", (coach[0]-price, code))
        cursor.execute("INSERT INTO admin_logs (team_name, log_type, message) VALUES (?, 'خرید مستقیم', ?)", (coach[1], f"خرید مستقیم {item}"))
        conn.commit()
        conn.close()
...
def submit_transfer():
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT team_name FROM coaches WHERE login_code=?", (code,))
    coach = cursor.fetchone()
    if coach:
        cursor.execute("INSERT INTO admin_logs (team_name, log_type, message) VALUES (?, 'نقل و انتقالات', ?)", (coach[1], msg))
        conn.commit()
    conn.close()
...
def create_match():
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO matches (team_a, team_b, tournament_name, description) VALUES (?, ?, ?, ?)", (t_a, t_b, t_name, desc))
    conn.commit()
    conn.close()
...
def submit_result():
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("UPDATE matches SET result=? WHERE id=?", (res, m_id))
    conn.commit()
    conn.close()
...
def upload_tour_image():
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("UPDATE tournaments SET stats_image=?, status='در حال برگزاری' WHERE name=?", (url, name))
    conn.commit()
    conn.close()
...
def update_permissions():
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("UPDATE account_permissions SET tournament_locked=?, league_locked=? WHERE account_type=?", (lock_tour, lock_league, target))
    conn.commit()
    conn.close()
...
def reg_tournament():
    ...
    coach = cursor.fetchone()
    if coach:
        cursor.execute("INSERT INTO tournament_regs (tournament_id, team_name) VALUES (?, ?)", (t_id, coach[0]))
        conn.commit()
    conn.close()
...
if __name__ == '__main__':
    init_db()
    # تغییر پورت برای خواندن مستقیم از متغیرهای محیطی Render
